...
def build_evals(conf):
    """
    Construit un dictionnaire d'évaluations pour différentes stratégies de prompting.
    ...
    if task_name != "linear_regression":
        # Pour certaines tâches, on peut ajouter la stratégie linéaire comme comparaison
        if task_name in ["relu_2nn_regression"]:
            evaluation_kwargs["linear_regression"] = {"task_name": "linear_regression"}

        # Merge avec les arguments de base
    ...
    if task_name == "matrix_completion":
        ...
        for n_pts in [int(n_points * f) for f in [0.5, 1.0, 1.5]]:
            evaluation_kwargs[f"n_points={n_pts}"] = {
                "task_sampler_kwargs": {"n_points": n_pts},
            }

        # Exemple : scaling / transformation linéaire sur M
        """
        for scale_factor in [0.5, 1.0, 2.0]:
            eigenvals = scale_factor * torch.ones(n_dims)
            t = sample_transformation(eigenvals)
            evaluation_kwargs[f"scale={scale_factor}"] = {
    ...
    # Merge avec base_kwargs pour toutes les évaluations
    for name, kwargs in evaluation_kwargs.items():
        evaluation_kwargs[name] = base_kwargs.copy()
        evaluation_kwargs[name].update(kwargs)

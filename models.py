...
def F_cvxpy(A, X1, X2):
    ...
    for j in range(X1.shape[0]):
        ...
        if isinstance(A, np.ndarray):
            outputs.append(float(x1_j @ A @ x2_j.T))
        else:
            outputs.append(x1_j @ A @ x2_j.T)
    if isinstance(A, np.ndarray):
    ...
    def __call__(self, xs, ys, inds=None):
        ...
        for i in inds:
            ...
            for b in range(batch_size):
                ...
                # Découpage dynamique
                # -----------------------------
                X1_train = xs[b, :i, :half].numpy().astype("float32")
                X2_train = xs[b, :i, half:].numpy().astype("float32")
                y_train = ys[b, :i].numpy().astype("float32").reshape(-1, 1)

                n1 = X1_train.shape[1]
                n2 = X2_train.shape[1]

                A_var = cp.Variable((n1, n2))
                F_expr = F_cvxpy(A_var, X1_train, X2_train)
                ...
                A_val = None if A_var.value is None else A_var.value

                x1_test = xs[b, i, :half].numpy().astype("float32").reshape(1, -1)
                x2_test = xs[b, i, half:].numpy().astype("float32").reshape(1, -1)

                if A_val is None:
                    batch_preds[b] = 0.0
                else:
                    batch_preds[b] = float(F_cvxpy(A_val, x1_test, x2_test).reshape(-1)[0])
